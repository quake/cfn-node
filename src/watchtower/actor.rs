...
use crate::{
    ...
    watchtower::store::SettlementData,
    NetworkServiceEvent,
};
                ...
                match event {
                    ...
                    NetworkServiceEvent::RemoteCommitmentSigned(
                        ...
                        self.store.update_settlement(
                            channel_id,
                            SettlementData {
                                settlement_tx: settlement_tx.data(),
                            },
                        );
                    }
                    _ => {
                                                    ...
                                                    if tx.raw().outputs().len() == 1 {
                                                        ...
                                                        .0 == pub_key_hash
                                                        {
                                                                ...
                                                                >= commitment_number
                                                            {
                                                                ...
                                                                info!("Found a force closed commitment tx: {:?}, revocation commitment number: {}, commitment number: {}", tx.calc_tx_hash(), revocation_data.commitment_number, commitment_number);
                                                            ...
                                                            // FIXME use 0xFE to get back the funds
                                                            // TODO use 0x00 ~ 0xFD to get back the funds
                                                        }
                                                    } else {
...
fn build_settlement_tx(
    ...
    let settlement_tx = &settlement_data.settlement_tx;
    let mut tx_builder = Transaction::default()
        .as_advanced_builder()
        .cell_deps(get_cell_deps(
            vec![Contract::CommitmentLock, Contract::Secp256k1Lock],
        ...
        .outputs(settlement_tx.raw().outputs())
        .outputs_data(settlement_tx.raw().outputs_data())
        .witnesses(settlement_tx.witnesses())
        .output(change_output.clone())
        .output_data(Bytes::default())
        .witness(placeholder_witness.as_bytes().pack());
    ...
    for cell in cells {
        ...
        if inputs_capacity >= change_output_occupied_capacity + fee {
            let new_change_output = change_output
                ...
                .capacity((inputs_capacity - fee).pack())
                .build();
            let mut outputs: Vec<CellOutput> = settlement_tx.raw().outputs().into_iter().collect();
            outputs.push(new_change_output);
